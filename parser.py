...
def _harmony_to_chord(h:ET.Element)->ChordPaint:
    ...
    name = step + kind
    fret0 = int(h.findtext('frame/first-fret','0'))
    ch = ChordPaint(name=name, fret=fret0)

    # mute все, потом open из frame-note
...
class GPXMLParser:
    ...
    def iter_chord_names(self):
        """Генератор: выдаёт уникальные имена аккордов для прогресса."""
        seen: Set[str] = set()
        for _, el in ET.iterparse(self.xml_path, events=('end',)):
            if el.tag != 'harmony':
                continue
            nm = _harmony_to_chord(el).name.name
            if nm not in seen:
                seen.add(nm)
                yield nm
            el.clear()
    ...
    def parse(self):
        """
        Полный рендер: draw_chord()/save_chord() —
        должен вызываться только в главном (GUI) потоке!
        """
