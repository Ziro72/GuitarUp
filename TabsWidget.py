...
class TabWidget(QDialog):
    def __init__(self):
        ...
        self.lineEditString_1.textChanged(lambda: self.string_status_changed(1))
        self.lineEditString_2.textChanged(lambda: self.string_status_changed(2))
        self.lineEditString_3.textChanged(lambda: self.string_status_changed(3))
        self.lineEditString_4.textChanged(lambda: self.string_status_changed(4))
        self.lineEditString_5.textChanged(lambda: self.string_status_changed(5))
        self.lineEditString_6.textChanged(lambda: self.string_status_changed(6))

        self.lineEditName.textChanged.connect(self.tabs_name_changed)

    def update_tab_menu(self):
        index = self.actual_row
        self.lineEditString_1.textChanged(self.tabs.get_cell_status((index, 1)))
        self.lineEditString_2.textChanged(self.tabs.get_cell_status((index, 2)))
        self.lineEditString_3.textChanged(self.tabs.get_cell_status((index, 3)))
        self.lineEditString_4.textChanged(self.tabs.get_cell_status((index, 4)))
        self.lineEditString_5.textChanged(self.tabs.get_cell_status((index, 5)))
        self.lineEditString_6.textChanged(self.tabs.get_cell_status((index, 6)))
    ...
    def tab_chord_changed(self):
        ...
        # self.tabs.set_name(self.actual_row, new_name)
        self.update_visual_display()

    def string_status_changed(self, index):
        string = eval(f"self.lineEditString_{index}")
        new_name = string.text()
        self.tabs.set_cell_status((self.actual_row, index), new_name)
        self.update_visual_display()
